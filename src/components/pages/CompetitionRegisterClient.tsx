...
export default function CompetitionRegisterPage() {
  ...
  useEffect(() => {
    if (regStatus && competition) {
      ...
      const savedRegion = draft?.region || regStatus.region;
      const savedGroupName = draft?.group_name || regStatus.group_name;
      const savedCategory = draft?.category || regStatus.category;
      ...
      } else if (regStatus.members && Array.isArray(regStatus.members) && regStatus.members.length > 1) {
         const dbMembers = regStatus.members.slice(1).map((m: any) => ({
             name: m.user?.name || '',
             email: m.user?.email || '',
             phone: m.user?.phone || '',
             id_card: null
         }));
         initialMembers = [...initialMembers, ...dbMembers];
      }
                      ...
                      {index === 0 ? (
                        regStatus?.user_profile?.ktm_path || regStatus?.user_profile?.id_card_path ? (
                          ...
                          <div>
                            ...
                            {member.preview && (
                              <div className="mt-4 border p-4" style={{ borderColor: palette.graphite, backgroundColor: palette.onyx }}>
                                <p className="text-[10px] font-bold mb-3 uppercase tracking-[0.2em]" style={{ color: palette.greige }}>PREVIEW:</p>
                                <img src={member.preview} alt="ID card preview" className="w-full max-w-md mx-auto rounded" style={{ border: `2px solid ${palette.graphite}` }} />
                              </div>
                            )}
                            {member.id_card && member.id_card.type === 'application/pdf' && (
                              <div className="mt-4 border p-4" style={{ borderColor: palette.graphite, backgroundColor: palette.onyx }}>
                                <p className="text-xs font-bold uppercase tracking-wider" style={{ color: palette.greige }}>✓ PDF FILE UPLOADED: {member.id_card.name}</p>
                              </div>
                            )}
                          </div>
                        )
