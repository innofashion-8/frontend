...
//           : 'https://chat.whatsapp.com/DVRPtov9lUxCKV9Kre08J8';
//       }
//       // Khusus Sketch, balikin null biar ga ada tombol WA
//       return null;
//     }
//     return null;
//   };
...
//         // WAJIB: Refresh data dashboard biar tombol langsung ganti jadi "VIEW SUBMISSION"
...
//   // HANDLER LIHAT KARYA (VIEW SUBMISSION)
//   const handleViewSubmission = (artworkUrl: string | null, conceptUrl: string | null) => {
//     if (!artworkUrl && !conceptUrl) {
//       Swal.fire({
//         title: 'SECURED', text: 'Your artwork has been securely stored in the system.', icon: 'success',
//         background: palette.onyx, color: palette.stucco, confirmButtonColor: palette.walnut,
//         customClass: { popup: 'border-2 border-[#494947] rounded-none shadow-[8px_8px_0px_#1a1a1a]', title: 'font-black tracking-[0.2em]' }
//       });
//       return;
//     }
...
//         </div>
//       `,
//       background: palette.onyx, color: palette.stucco, confirmButtonText: 'CLOSE', confirmButtonColor: palette.graphite,
//       customClass: { popup: 'border-2 border-[#494947] rounded-none shadow-[8px_8px_0px_#1a1a1a]', title: 'font-black tracking-[0.2em] uppercase', confirmButton: 'font-bold tracking-widest uppercase rounded-none px-6 py-2' }
//     });
//   };
...
//                 // Get WA link (Skrg khusus restyling & event aja yg dapet link)
//                 const finalWaLink = getWhatsAppLink(reg); 
...
//                 // 🔥 LOGIKA CEK SUBMISSION (UPDATED BUAT ARRAY DARI CLARENCE) 🔥
//                 const submissionObj = reg?.submissions?.[0] || reg?.submission || reg?.competition_submission; 
//                 const hasSubmitted = !!submissionObj?.artwork_path || !!reg?.artwork_path; 
                
//                 // Generate URL file buat di View
//                 const baseUrl = process.env.NEXT_PUBLIC_API_URL || '';
//                 const artworkUrl = submissionObj?.artwork_path ? `${baseUrl}/storage/${submissionObj.artwork_path}` : null;
//                 const conceptUrl = submissionObj?.concept_path ? `${baseUrl}/storage/${submissionObj.concept_path}` : null;
...
//                         {/* 🔥 TOMBOL UPLOAD KARYA (HANYA SKETCH VERIFIED & BLM SUBMIT) 🔥 */}
...
//                         {/* 🔥 TOMBOL VIEW KARYA (HANYA SKETCH VERIFIED & UDAH SUBMIT) 🔥 */}
//                         {isAccepted && isSketchComp && hasSubmitted && (
//                             <button 
...
//                         {/* 🔥 TOMBOL WHATSAPP (HANYA EVENT / RESTYLING VERIFIED) 🔥 */}
//                         {isAccepted && finalWaLink && (isEvent || isRestylingComp) && (
//                           <a 
//                             href={finalWaLink} target="_blank" rel="noopener noreferrer"
//                             className="px-4 py-2 border font-bold text-[10px] tracking-widest uppercase bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366] hover:text-white transition-all cursor-pointer"
...
export default function DashboardClient() {
  ...
  const handleUploadSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // Minta React Query buat nyedot data terbaru dari Clarence (yg udah ada array submissions-nya)
        queryClient.invalidateQueries({ queryKey: ['my-registrations'] });
  ...
  const handleViewSubmission = (artworkUrl: string | null, conceptUrl: string | null) => {
    if (!artworkUrl && !conceptUrl) {
      Swal.fire({
        title: 'PROCESSING', text: 'Your artwork is being processed by the server. Please check again later.', icon: 'info',
        background: palette.onyx, color: palette.stucco, confirmButtonColor: palette.walnut,
        customClass: { popup: 'border-2 border-[#494947] rounded-none shadow-[8px_8px_0px_#1a1a1a]', title: 'font-black tracking-[0.2em]' }
      });
      return;
          ...
          {isRegLoading ? (
            ...
            <div className="space-y-4">
              {allRegistrations.map((reg, idx) => {
                ...
                // 🔥 LOGIKA SAKTI BACA DATA DARI CLARENCE 🔥
                const submissionsArray = reg?.submissions || [];
                const hasSubmitted = submissionsArray.length > 0;
                
                const artworkObj = submissionsArray.find((s: any) => s.file_type === 'ARTWORK');
                const conceptObj = submissionsArray.find((s: any) => s.file_type === 'CONCEPT');
