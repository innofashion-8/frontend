...
export default async function Page({ searchParams }: { searchParams: Promise<{ page?: string; search?: string; event_name?: string; user_type?: string; attended_status?: string }> }) {
  const params = await searchParams;
  const page = parseInt(params.page || "1");
  const search = params.search || "";
  const event_name = params.event_name || "";
  const user_type = params.user_type || "";
  const attended_status = params.attended_status || "";
